...
async function igDownload(url) {
	const methods = [
		// Method 1: instaloader via yt-dlp
		async () => {
		...
		// Method 2: instagramsave API
		async () => {
			const res = await fetch(`https://instasave.website/api`, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ url }), signal: AbortSignal.timeout(15000) });
			const data = await res.json();
		...
		// Method 3: saveig.app
		async () => {
			const res = await fetch('https://v3.saveig.app/api/ajaxSearch', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'X-Requested-With': 'XMLHttpRequest' }, body: new URLSearchParams({ q: url, t: 'media', lang: 'en' }), signal: AbortSignal.timeout(15000) });
			const data = await res.json();
		...
		// Method 4: snapinsta
		async () => {
			const r1 = await fetch('https://snapinsta.app/', { signal: AbortSignal.timeout(10000) });
			const html1 = await r1.text();
			const token = html1.match(/name="_token"\s+value="([^"]+)"/)?.[1];
			if (!token) throw new Error('no token');
			const r2 = await fetch('https://snapinsta.app/action.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'X-Requested-With': 'XMLHttpRequest' }, body: new URLSearchParams({ url, _token: token }), signal: AbortSignal.timeout(20000) });
	...
	return await runMethods(methods.map((fn, i) => ({ name: `ig-method-${i + 1}`, fn })), 'Instagram');
}
...
async function tiktokDownload(url) {
	const methods = [
		...
		// Method 2: tikwm.com
		async () => {
			const res = await fetch('https://tikwm.com/api/', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ url, count: '12', cursor: '0', web: '1', hd: '1' }), signal: AbortSignal.timeout(15000) });
			const data = await res.json();
			...
			return { type: 'video', url: d.hdplay || d.play, audio: d.music, title: d.title || '', author: d.author?.nickname || '', thumb: d.cover || '' };
		},
		// Method 3: ssstik.io
		async () => {
			...
			if (!token) throw new Error('no token');
			const r2 = await fetch('https://ssstik.io/abc?url=dl', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://ssstik.io/en' }, body: new URLSearchParams({ id: url, locale: 'en', tt: token }), signal: AbortSignal.timeout(20000) });
			const html2 = await r2.text();
			const dlUrl = html2.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"/)?.[1];
			const audioUrl = html2.match(/href="(https:\/\/[^"]+\.mp3[^"]*)"/)?.[1];
			if (!dlUrl) throw new Error('no link');
			return { type: 'video', url: dlUrl, audio: audioUrl || dlUrl, title: '', author: '', thumb: '' };
		},
		// Method 4: musicaldown
		async () => {
			const r1 = await fetch('https://musicaldown.com/en', { signal: AbortSignal.timeout(10000) });
			const html = await r1.text();
			const inputs = [...html.matchAll(/<input[^>]+name="([^"]+)"[^>]+value="([^"]*)"/g)].reduce((a, m) => ({ ...a, [m[1]]: m[2] }), {});
			inputs.link = url;
			const r2 = await fetch('https://musicaldown.com/download', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://musicaldown.com/en' }, body: new URLSearchParams(inputs), signal: AbortSignal.timeout(20000) });
			const html2 = await r2.text();
			const dlUrl = html2.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"/)?.[1];
			if (!dlUrl) throw new Error('no link');
	...
	return await runMethods(methods.map((fn, i) => ({ name: `tt-method-${i + 1}`, fn })), 'TikTok');
}
...
async function fbDownload(url) {
	...
	return await runMethods(methods.map((fn, i) => ({ name: `fb-method-${i + 1}`, fn })), 'Facebook');
}
