...
async function tiktokDownload(url) {
	...
	let resolvedUrl = url;
	if (url.includes('vm.tiktok') || url.includes('vt.tiktok') || !url.includes('/video/')) {
		try {
			const r = await fetch(url, { method: 'HEAD', redirect: 'follow', signal: AbortSignal.timeout(10000), headers: { 'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)' } });
			resolvedUrl = r.url || url;
		} catch { resolvedUrl = url; }
	}

	const methods = [
		// Method 1: tikwm.com — best no-watermark API
		{ name: 'tikwm', fn: async () => {
			const res = await fetch('https://tikwm.com/api/', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0' }, body: new URLSearchParams({ url: resolvedUrl, count: '12', cursor: '0', web: '1', hd: '1' }), signal: AbortSignal.timeout(20000) });
			const data = await res.json();
		...
		{ name: 'snaptik', fn: async () => {
			const r1 = await fetch('https://snaptik.app/en', { signal: AbortSignal.timeout(10000) });
			const html1 = await r1.text();
			const token = html1.match(/name="token"\s+value="([^"]+)"/)?.[1];
			if (!token) throw new Error('no token');
			const r2 = await fetch('https://snaptik.app/action_v2.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://snaptik.app/' }, body: new URLSearchParams({ url: resolvedUrl, token, lang: 'en' }), signal: AbortSignal.timeout(20000) });
			const data = await r2.json();
		...
		{ name: 'ssstik', fn: async () => {
			const r1 = await fetch('https://ssstik.io/en', { signal: AbortSignal.timeout(10000) });
			const html = await r1.text();
			const token = html.match(/s_tt\s*=\s*"([^"]+)"/)?.[1];
			if (!token) throw new Error('no token');
			const r2 = await fetch('https://ssstik.io/abc?url=dl', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://ssstik.io/en' }, body: new URLSearchParams({ id: resolvedUrl, locale: 'en', tt: token }), signal: AbortSignal.timeout(25000) });
		...
		// Method 4: musicaldown
		{ name: 'musicaldown', fn: async () => {
			const r1 = await fetch('https://musicaldown.com/en', { headers: { 'User-Agent': 'Mozilla/5.0' }, signal: AbortSignal.timeout(10000) });
			const html = await r1.text();
			const inputs = [...html.matchAll(/<input[^>]+name="([^"]+)"[^>]+value="([^"]*)"/g)].reduce((a, m) => ({ ...a, [m[1]]: m[2] }), {});
			inputs.link = resolvedUrl;
			const r2 = await fetch('https://musicaldown.com/download', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://musicaldown.com/en' }, body: new URLSearchParams(inputs), signal: AbortSignal.timeout(25000) });
			const html2 = await r2.text();
			const dlUrl = html2.match(/href="(https:\/\/[^"]+\.mp4[^"]*)"/)?.[1];
			if (!dlUrl) throw new Error('no link');
			return { type: 'video', url: dlUrl, audio: dlUrl, title: '', author: '', thumb: '' };
		...
		// Method 5: tikcdn API
		{ name: 'tikcdn', fn: async () => {
			const res = await fetch(`https://api22-normal-c-alisg.tiktokv.com/aweme/v1/feed/?aweme_id=${resolvedUrl.match(/\/video\/(\d+)/)?.[1]}&iid=7318518857994389254&device_id=7318517321748022790&channel=googleplay&app_name=musical_ly&version_code=300904&device_platform=android&device_type=Pixel+7&version=9`, { headers: { 'User-Agent': 'okhttp/4.9.0', 'accept': 'application/json' }, signal: AbortSignal.timeout(20000) });
			const data = await res.json();
		...
		// Method 6: yt-dlp (most reliable fallback)
		{ name: 'ytdlp', fn: async () => {
			const out = await run(`yt-dlp --no-warnings -J "${resolvedUrl}"`, 45000);
			const info = JSON.parse(out);
		...
		{ name: 'cobalt', fn: async () => {
			...
			for (const inst of instances) {
				try {
					const r = await fetch(`${inst}/`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ url: resolvedUrl, downloadMode: 'auto', videoQuality: '720' }), signal: AbortSignal.timeout(15000) });
					const d = await r.json();
		...
		// Method 8: savefrom net API
		{ name: 'savefrom', fn: async () => {
			const res = await fetch('https://worker.sf-tools.com/savefrom.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'Mozilla/5.0', 'Referer': 'https://en.savefrom.net/' }, body: new URLSearchParams({ sf_url: resolvedUrl, lang: 'en' }), signal: AbortSignal.timeout(20000) });
			const data = await res.json();
			const url_ = data?.url?.[0]?.url || data?.url;
			if (!url_) throw new Error('no url');
			return { type: 'video', url: url_, audio: url_, title: data?.title || '', author: '', thumb: data?.thumb || '' };
		}},
	];
	...
	for (const method of methods) {
		try {
			...
			console.log(`[TikTok] ❌ ${method.name}: ${e.message}`);
		}
	}
